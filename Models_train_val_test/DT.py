...
# Initialize scaler (optional for DT, but kept for consistency)
...
modelval_DT = []       # Store trading system values over time
y_test_window_DT = []  # Store all test predictions concatenated
feature_importance_history = []  # Store feature importance across windows
tree_rules_history = []         # Store tree rules/splits for interpretation
...
# ============================================================================
print("\n" + "="*80)
print("STARTING SLIDING WINDOW ANALYSIS WITH DECISION TREE AND DYNAMIC CROSS-VALIDATION")
print("="*80 + "\n")
...
for window_idx in range(num_windows):
    ...
    try:
        ...
        # Use default DT for SFFS
        sfs = SFS(
            DecisionTreeClassifier(criterion='gini', max_depth=3, max_features='sqrt', random_state=0),
            k_features=(1, 20),
            forward=True,
            floating=True,
            verbose=0,
    ...
    # ========================================================================
    print("\n" + "="*60)
    print(f"DECISION TREE PARAMETER OPTIMIZATION - WINDOW {window_idx + 1}")
    print("="*60 + "\n")
    ...
    for param_idx, params in enumerate(param_combinations):
        ...
        for fold in range(1, cv_folds):
            ...
            try:
                ...
                print(f"  Warning: DT failed for params {params} on fold {fold}: {str(e)}")
        ...
        # Progress update - DT is fast, so we can update more frequently
        if (param_idx + 1) % 20 == 0:
    ...
    if cv_results:
        ...
        print(f"\n{'='*60}")
        print(f"BEST DECISION TREE PARAMETERS FOUND:")
        print(f"{'='*60}")
    ...
    if best_fold_accs:
        ...
        ax.set_title(f'Window {window_idx + 1}: DT CV Results ({len(best_fold_accs)} folds)', fontsize=18)
    ...
    # ========================================================================
    print("\n--- Training Final Decision Tree with Optimized Parameters ---")
    ...
    try:
        ...
        model_dt.fit(X_train_final_scaled, y_train_final)
        print("Decision Tree model trained successfully")
        
    except Exception as e:
        print(f"Warning: DT failed to train with best params: {str(e)}")
        print("Using fallback parameters...")
        
        # Fallback to simple DT
        fallback_params = {
            'criterion': 'gini',
            'max_depth': 3,
            'min_samples_split': 2,
        ...
        model_dt = DecisionTreeClassifier(**fallback_params)
        model_dt.fit(X_train_final_scaled, y_train_final)
    
    # Make predictions
    y_pred_test = model_dt.predict(X_test_final_scaled)
    y_pred_train = model_dt.predict(X_train_final_scaled)
    y_pred_prob_test = model_dt.predict_proba(X_test_final_scaled)[:, 1]
    ...
    if hasattr(model_dt, 'feature_importances_') and len(model_dt.feature_importances_) > 0:
        feature_importance = dict(zip(final_features, model_dt.feature_importances_))
    ...
    # Extract and store tree rules for interpretation
    try:
        tree_rules = export_text(model_dt, feature_names=list(final_features), max_depth=3)
        tree_rules_history.append({
            'window': window_idx,
            'rules': tree_rules,
            'best_params': best_params
        })
    except:
        print("Could not extract tree rules")
    ...
    if hasattr(model_dt, 'feature_importances_') and len(model_dt.feature_importances_) > 0:
        feature_importance_list = list(zip(final_features, model_dt.feature_importances_))
        feature_importance_list.sort(key=lambda x: x[1], reverse=True)
        for feature, importance in feature_importance_list[:10]:
            print(f"  {feature}: {importance:.4f}")
    
    # Visualize decision tree for first window only (to avoid too many plots)
    if window_idx == 0:
        print("\nVisualizing Decision Tree (First Window Only)...")
        ...
        plt.title(f'Decision Tree - Window {window_idx + 1} (Optimized)', fontsize=20)
        plt.tight_layout()
        plt.show()
...
for i in range(num_windows):
    y_test_window_DT.extend(yptt[i])

print(f"Total predictions: {len(y_test_window_DT)}")
...
if cv_folds_history:
    ...
    ax.bar(windows, cv_folds_history, color='darkorange', edgecolor='black', alpha=0.7)
    ...
    ax.set_title('Number of Cross-Validation Folds per Window (Decision Tree)', fontsize=14)
    ...
    print("\nCV Folds Summary for Decision Tree:")
...
conf_matrix = confusion_matrix(y_true, y_test_window_DT)
acc = accuracy_score(y_true, y_test_window_DT)
precision = precision_score(y_true, y_test_window_DT)
recall = recall_score(y_true, y_test_window_DT)
f1 = f1_score(y_true, y_test_window_DT)
mcc = matthews_corrcoef(y_true, y_test_window_DT)
...
print(f"\n{classification_report(y_true, y_test_window_DT, digits=4)}")
...
sns.heatmap(conf_matrix, annot=labels, linewidths=1, fmt='', 
            cmap='Oranges', annot_kws={"size": 14})
ax.set_xlabel('Predicted Labels', fontsize=15)
ax.set_ylabel('Actual Labels', fontsize=15)
ax.set_title('Decision Tree - Confusion Matrix (With Dynamic CV)', fontsize=16)
plt.tight_layout()
plt.show()

# ============================================================================
# DECISION TREE PARAMETER OPTIMIZATION SUMMARY WITH DYNAMIC CV
# ============================================================================
print("\n" + "="*80)
print("DECISION TREE PARAMETER OPTIMIZATION SUMMARY WITH DYNAMIC CV")
print("="*80 + "\n")
...
criterion_stats = {}
max_depth_stats = {}
min_samples_leaf_stats = {}
min_samples_split_stats = {}
max_features_stats = {}
class_weight_stats = {}
...
for window_idx, params in enumerate(best_params_history):
    ...
    min_samples_leaf_stats[min_samples_leaf] = min_samples_leaf_stats.get(min_samples_leaf, 0) + 1
    min_samples_split_stats[min_samples_split] = min_samples_split_stats.get(min_samples_split, 0) + 1
...
# Print statistics
print("\n" + "="*60)
print("DECISION TREE PARAMETER SELECTION STATISTICS WITH DYNAMIC CV:")
print("="*60)
...
print("\nMinimum Samples per Leaf Selection:")
for samples, count in sorted(min_samples_leaf_stats.items(), key=lambda x: x[1], reverse=True):
    percentage = (count / num_windows) * 100
    print(f"  {samples}: {count} windows ({percentage:.1f}%)")

print("\nMinimum Samples to Split Selection:")
for samples, count in sorted(min_samples_split_stats.items(), key=lambda x: x[1], reverse=True):
    percentage = (count / num_windows) * 100
    print(f"  {samples}: {count} windows ({percentage:.1f}%)")
...
axes[0, 0].bar(criteria, criterion_counts, color='orange')
axes[0, 0].set_xlabel('Splitting Criterion', fontsize=12)
axes[0, 0].set_ylabel('Frequency', fontsize=12)
axes[0, 0].set_title('Splitting Criterion Selection', fontsize=14)
...
axes[0, 1].bar(depth_labels, depth_counts, color='darkorange')
axes[0, 1].set_xlabel('Maximum Depth', fontsize=12)
axes[0, 1].set_ylabel('Frequency', fontsize=12)
axes[0, 1].set_title('Maximum Depth Selection', fontsize=14)
axes[0, 1].grid(True, alpha=0.3, axis='y')

# Min samples leaf selection
samples_leaf, leaf_counts = zip(*sorted(min_samples_leaf_stats.items(), key=lambda x: x[1], reverse=True))
axes[0, 2].bar([str(s) for s in samples_leaf], leaf_counts, color='gold')
axes[0, 2].set_xlabel('Min Samples per Leaf', fontsize=12)
axes[0, 2].set_ylabel('Frequency', fontsize=12)
axes[0, 2].set_title('Minimum Samples per Leaf', fontsize=14)
axes[0, 2].grid(True, alpha=0.3, axis='y')

# Min samples split selection
samples_split, split_counts = zip(*sorted(min_samples_split_stats.items(), key=lambda x: x[1], reverse=True))
axes[1, 0].bar([str(s) for s in samples_split], split_counts, color='coral')
axes[1, 0].set_xlabel('Min Samples to Split', fontsize=12)
axes[1, 0].set_ylabel('Frequency', fontsize=12)
axes[1, 0].set_title('Minimum Samples to Split', fontsize=14)
...
axes[1, 1].bar(feature_labels, feature_counts, color='peachpuff')
...
# CV Folds Evolution
windows = np.arange(1, num_windows + 1)
axes[1, 2].plot(windows, cv_folds_history, 'o-', linewidth=2, markersize=8, color='darkorange')
axes[1, 2].set_xlabel('Window', fontsize=12)
axes[1, 2].set_ylabel('Number of CV Folds', fontsize=12)
axes[1, 2].set_title('CV Folds Evolution (Decision Tree)', fontsize=14)
...
if feature_importance_history:
    ...
    ax1.barh(y_pos, importances, color='darkorange')
    ax1.set_yticks(y_pos)
    ax1.set_yticklabels(features)
    ax1.invert_yaxis()
    ax1.set_xlabel('Average Feature Importance', fontsize=12)
    ax1.set_title('Top 15 Feature Importance - Decision Tree', fontsize=14)
...
# ============================================================================
# TREE COMPLEXITY ANALYSIS WITH DYNAMIC CV
# ============================================================================
print("\n" + "="*80)
print("DECISION TREE COMPLEXITY ANALYSIS WITH DYNAMIC CV")
print("="*80 + "\n")

print("Most Frequently Selected Parameters:")
print("-" * 50)

# Find most common parameters
most_common_criterion = max(criterion_stats.items(), key=lambda x: x[1])[0]
most_common_depth = max(max_depth_stats.items(), key=lambda x: x[1])[0]
most_common_leaf = max(min_samples_leaf_stats.items(), key=lambda x: x[1])[0]
most_common_split = max(min_samples_split_stats.items(), key=lambda x: x[1])[0]

print(f"1. Splitting criterion: {most_common_criterion}")
print("   - Gini: faster computation, similar results to entropy")
print("   - Entropy: information gain based, slightly more precise")
print(f"2. Maximum depth: {most_common_depth}")
print("   - Controls tree complexity, prevents overfitting")
print(f"3. Minimum samples per leaf: {most_common_leaf}")
print("   - Ensures leaves have sufficient samples for reliable predictions")
print(f"4. Minimum samples to split: {most_common_split}")
print("   - Requires enough samples before creating new splits")

print("\nDynamic CV Analysis:")
print(f"CV folds range: {min(cv_folds_history)} to {max(cv_folds_history)}")
print(f"Average CV folds per window: {np.mean(cv_folds_history):.1f}")

print("\nTree Complexity Insights:")
print(f"The optimized parameters suggest trees that are:")
print(f"- {'' if most_common_depth is None else 'Shallow' if most_common_depth <= 5 else 'Moderate' if most_common_depth <= 10 else 'Deep'}")
print(f"- {'More regularized' if most_common_leaf > 5 else 'Less regularized'}")
print(f"- {'Balanced' if most_common_split >= 10 else 'More flexible'}")

print("\nBenefits of Dynamic CV for Decision Trees:")
print("1. More reliable parameter tuning with increasing data")
print("2. Better prevention of overfitting with more validation")
print("3. Adapts to available historical data")
print(f"4. Ensures robust validation from {min(cv_folds_history)} to {max(cv_folds_history)} folds")

# Display sample tree rules
if tree_rules_history:
    print("\n=== Sample Decision Tree Rules (First Window) ===")
    print(tree_rules_history[0]['rules'])

# ============================================================================
# TRADING SIMULATION
# ============================================================================
print("\n" + "="*80)
print("TRADING SIMULATION WITH OPTIMIZED DECISION TREE AND DYNAMIC CV")
print("="*80 + "\n")
...
total_return = ((modelval_DT[-1] - initial_capital) / initial_capital) * 100

print(f"{'='*60}")
print(f"TRADING RESULTS WITH OPTIMIZED DECISION TREE AND DYNAMIC CV")
print(f"{'='*60}")
print(f"Initial Investment:     ${initial_capital:.2f}")
print(f"Final Investment Value: ${modelval_DT[-1]:.2f}")
print(f"Total Return:           {total_return:.2f}%")
print(f"Absolute Profit/Loss:   ${modelval_DT[-1] - initial_capital:.2f}")
print(f"{'='*60}\n")

plt.figure(figsize=(12, 6))
plt.plot(modelval_DT, label='Investment Value', linewidth=2, color='darkorange')
plt.axhline(y=initial_capital, color='r', linestyle='--', label='Initial Investment')
plt.xlabel('Trading Day')
plt.ylabel('Investment Value ($)')
plt.title('Trading System Performance with Optimized Decision Tree (Dynamic CV)')
plt.legend()
...
df_modelval_DT = pd.DataFrame(modelval_DT)
num_years = num_windows
aar = (((modelval_DT[-1] / modelval_DT[0]) ** (1 / num_years)) - 1) * 100
print(f"Average Annual Return (AAR): {aar:.2f}%")

mdd_series = max_drawdown(df_modelval_DT)
...
modelval_DT_list = modelval_DT.copy()
modelval_DT_list.insert(0, initial_capital)

port_value = pd.DataFrame(modelval_DT_list, columns=['Value'])
...
for year_idx in range(num_years):
    ...
    annal_return_DT.append(annual_ret)

annal_return_DT_df = pd.DataFrame(annal_return_DT, columns=['Return'])
...
if std_neg > 0:
    sortino = ((((modelval_DT_list[-1] / modelval_DT_list[0]) ** (1 / num_years)) - 1) - riskfree) / (std_neg * np.sqrt(252))
    print(f"Sortino Ratio: {sortino:.4f}")

annual_mean_return = (((modelval_DT_list[-1] / modelval_DT_list[0]) ** (1 / num_years)) - 1)
annual_return_std = annal_return_DT_df['Return'].std()
